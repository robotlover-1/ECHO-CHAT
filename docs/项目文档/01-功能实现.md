...
```
值布局（自定义二进制）: [u32 dim][float vec[dim]]              小端   （2026-09 简化，去掉了 q/a 段）
...
- **值布局**：`[u32 dim][float vec[dim]]`（小端）。2026-09 从旧的 `[qlen][q][alen][a][dim][vec]` 简化而来——**向量 value 不再内嵌问题/答案**，回答正文单独以明文 `SET <问题原文> = <回答>` 存数组引擎，VSEARCH 命中后由上层另发 `GET <问题原文>` 取回答（见文档 4 §11.3）。
...
- **业务形态（语义缓存命中）**：ai-chat 侧把问题嵌入成向量存 `semcache:<问题原文>`（hash 引擎），LLM 请求来时先用 `VSEARCH` 查库里语义最接近的旧问题，命中（得分够高、过 rerank）就 `GET <问题原文>` 返回缓存答案，省一次 LLM 调用。见文档 4。

---

> **❓ 追问：topk 就是得分排名吗？分数怎么算？`semcache:<key>` 的 key 是什么、value 是什么？**
>
> 以实际调用方 ai-chat 的 `semcache.go` 为准（**2026-09 已改为明文形态，见文档 4 §11.3**）：
>
> - **key** = `semcache:<问题原文>`（早期是 `semcache:<FNV-32a(query) 8 位十六进制>`，已去掉 hash）。同一问题 → 同 key → 天然去重。
> - **value** = `[u32 dim][float32 vec[dim]]`（`encodeVec`，`semcache.go`）。**只存向量**；问题原文就是 key，回答正文另以明文 `SET <问题原文> = <回答>` 存**数组引擎**。写入 `HSET semcache:<问题原文> <[dim][vec]>`——**必须 HSET（hash 引擎）**，因为 `VSEARCH` 只扫 `global_hash`。
> - **topk**：`VSEARCH` 返回得分最高的前 k 个（`CacheQuery` 请求 `VSEARCH 256 <query_vec> 5`）。**得分 = 余弦相似度** `dot(a,b)/(|a|·|b|)`（`kvs_vector.c:33-42`），∈[0,1]。
> - **命中判定（`CacheQuery`）**：对 VSEARCH 返回的 **topK 逐条**校验——`score >= Threshold` → `GET <缓存问题>` 取明文回答 → `/rerank`（主题/意图/语言冲突硬拒 + 关键词 Jaccard，均通过才命中）。Top1 被拒则试 Top2（2026-09 起从"只取 Top1"改为 TopK 逐条）。
>
> **❓ 追问：向量是什么？为什么余弦能判相似？rerank 又是什么？**
>
> **向量 = 加权词面嵌入，不是神经网络**：tokenizer `/embed` 用 jieba 分词 + 字符 bigram **哈希到 256 维桶**，并做加权——主题词（`SUBJECT:红黑树`）×3.0、意图词（`INTENT:implementation`）×2.0、普通词 ×1.0、模板词（一个/请问…）×0。语义相近的改写句共享词面 → 方向接近；不同主题/意图靠主题、意图标记桶拉开（"红黑树是什么 vs 数组是什么"余弦 0.57→0.014）。
>
> **为什么余弦能判相似**：两个向量夹角越小 → cos 越接近 1 → 词面/主题越接近。这是"词面重叠检索"，不是真语义理解——故上层用主题/意图/语言规则 + 准入层补安全边界（见文档 4 §11.7）。
>
> **rerank = 规则启发式，不是交叉编码器**：`/rerank` 对 `(query, 缓存问题)` 依次做：①主题冲突硬拒（`extract_subject` 抽到的主题不同 → 拒）；②意图冲突硬拒（`extract_intent`，实现≠定义 → 拒）；③语言冲突（Go vs Python → 拒）；④关键词 Jaccard ≥ rerank_threshold。**缓存准入层**（`should_bypass`）：上下文依赖（之前/继续）或状态修改指令（记住/你是一个/扮演）**直接不查不写**缓存。真语义理解需换真实 embedding 模型（BGE），规则仍保留作安全边界。
...
正文 🔍 5.2 答了"怎么用"。补业务形态：这是**进程级访问控制**而非应用层鉴权——典型场景是 kvstore 作为 ai-chat 的"短期记忆"库（多轮上下文 + 语义缓存）时，**不让外部/其他服务随意读写**。门禁在 `handle_parsed_command` 最前意味着未鉴权连 `PING`/`ECHO` 都拦（与 Redis 语义一致）；内部复制连接（`from_replication=1`）豁免。ai-chat 早期连 kvstore 配 `requirepass`；**2026-09 起开发环境已去掉 requirepass（本地免密直连，`redis-cli GET <问题>` 可直接读）**，AUTH 命令能力保留。
...
*文档生成于 2026-08-26，基于当前代码逐行核验；2026-08-27 补充各章"🔍 深入解析"FAQ；2026-08-28 追加 §17 逐题核验与深入。2026-09-03 更新 §6.6 Vector/VSEARCH——record 简化为 `[dim][vec]`、语义缓存 key 去 hash 改为明文 `semcache:<问题原文>`、rerank 澄清为规则启发式（非交叉编码器）。存储引擎、过期、持久化、AOF group-commit、backlog、WRITE/sendfile 回退、转发线程等模块与代码一致。*
