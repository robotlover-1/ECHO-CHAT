...
```
1. 问题 → semantic /embed → 256 维加权向量（含 SUBJECT/INTENT 标记桶）
...
**rerank（semantic `/rerank`，规则启发式，非交叉编码器）**：依次做
1. **主题冲突硬拒**：`extract_subject`（定义"什么是X/X是什么"、实现"实现X/写X/生成X/用语言写X"、操作"X的插入"等正则抽主语）——**双方都有主题且互不包含**才拒（`_subject_conflict`：数组 ⊂ "数组的实现" → 兼容；数组 vs 红黑树 → 拒）；
2. **意图冲突硬拒**：`extract_intent`（state_update/history/implementation/definition/comparison/reason/operation/troubleshooting）——**仅双方都可判定且不同**才拒（"定义 vs 实现"）；history_query 一律拒；unknown 侧放行给向量+主题+关键词决定（否则"生成一个红黑树"等未收录意图问题连精确重问都被误拒）；
3. **语言冲突**：`extract_language`（Go vs Python）不同 → 拒；
4. **操作冲突**：`extract_operation`（插入/删除/遍历/查询…）同为操作句但不同 → 拒（"红黑树的插入" vs "红黑树的删除"）；
5. **关键词 Jaccard** ≥ `rerank_threshold(0.25)`。

**缓存准入层（安全收窄，方案 A）**——CacheQuery/CacheWrite 都跳过：
- **状态修改/上下文依赖**（`should_bypass`）：记住/你是一个/之前/继续… → 不查不写；
- **抽不到明确主题**（`/embed` 返回 `subject=""`）："怎么减肥/推荐电影/怎么办"这类通用 how-to 无法正则切主题 → **不缓存、不查缓存，永远走 LLM**——杜绝"怎么减肥 vs 怎么学好英语"类跨主题误命中。软件/代码/定义题（subject 可靠）照常缓存。
...
- **/embed 嵌入（256 维加权词面，FNV 哈希）**：`embed_text`——①主题词 `SUBJECT:<主题>`×3.0 + 意图词 `INTENT:<意图>`×2.0（标记桶，拉开不同主题/意图）；②jieba 词 ×1.0（模板词"一个/请问/可以"×0）；③相邻字符 bigram ×0.4；L2 归一化。稀疏、无监督、可重复。**非神经网络**（§11.7）。
