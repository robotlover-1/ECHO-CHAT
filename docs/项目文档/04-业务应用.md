...
为支撑 go-redis 客户端接入，kvstore **新增了 `AUTH`、`SETEX` 命令，并修复了 `CLIENT` 子命令大小写兼容问题**。此外实现了 `VSEARCH` 命令（`src/storage/kvs_vector.c`，暴力余弦 top-k），配合 semantic 的 `/embed`（加权词面嵌入 256 维）与 `/rerank`（主题/意图/语言冲突硬拒），实现**跨对话的语义缓存命中——命中后直接"抄答案"、不调大模型，秒回且免费**。
...
| 3 | **semantic** | Python | 3003 / HTTP | 语义检索 /embed /rerank（256 维词面嵌入，2026-09 自 tokenizer 迁入，见 §6.2） |
| 4 | **sensitive 敏感词** | Go | 50053 / gRPC | AC 自动机，命中即拦截回答 |
| 5 | **keyword 关键词** | Go | 50054 / gRPC | 提取关键词（记录 + rerank 校验） |
| 6 | **mock-openai-api** | Go | 8083 / HTTP | 模拟 OpenAI，无真实 key 时用 |
| 7 | **openai-api-proxy** | Go | 8084 / HTTP | 反向代理 + 鉴权 + 限流，集中管真 DeepSeek key |
| 8 | **ai-chat-service** | Go | 50055 / gRPC(流式) | 核心编排：敏感词→语义缓存→上下文→token 预算→调模型→流式转发 |
...
flowchart TB
    ...
    subgraph 数据层
        ...
        T["semantic :3003 /embed /rerank（语义检索）<br/>tokenizer :3002 计 token"]
...
| 3 | **semantic** | Python `semantic.py`（nuxt） | 3003 / HTTP | service（`/embed` / `/rerank` / `/healthz`） |
| 4 | **sensitive** | Go `keywords-filter`（敏感词典） | 50053 / gRPC | service（Validate） |
| 5 | **keyword** | Go `keywords-filter`（关键词词典） | 50054 / gRPC | service（FindAll） |
...
| 8 | **service** | Go `ai-chat-service` | 50055 / gRPC 流式 | backend（ChatCompletionStream） |
...
| service → semantic | HTTP | 无鉴权 | 内网地址可达即用（语义检索，`/embed` / `/rerank`） |
...
sequenceDiagram
    ...
    alt 敏感词命中
        ...
        SVC->>SEM: ⑧ /embed 问题 → 256维(含SUBJECT/INTENT标记桶 + bypass_cache/intent/subject)
...
> ⚠ **2026-09 形态已改**（§11.3/§11.7 + semantic Phase1）：**明文 Q-A + 独立向量索引 + 语义指纹候选前置**。以下为当前代码事实。

**写入流程（CacheWrite，聊天后异步、content 非空且非截断时）**：
```
1. 问题 → semantic /embed → 256 维加权向量 + 结构化槽位（双路subject_id/意图/语言/操作/residual/指纹）
2. SET <问题原文> = <回答原文>                     # 数组引擎，明文（key/value 均可 redis-cli 直读）
3. HSET semcache:<问题原文> = [u32 dim][float32 vec[dim]]   # hash 引擎向量索引（VSEARCH 候选回退路径）
4. SET semfp:v1:<fingerprint> = <候选问题原文>      # 安全指纹 → 只存候选问题指针，不存答案（见下）
```
...
```mermaid
flowchart TB
    A["CacheQuery(问题)"] --> B{"缓存准入 bypass_cache/无主题(miss)?"}
    B -- 上下文依赖/状态修改/抽不到主题 --> X["绕过→LLM"]
    B -- 否 --> C["/embed → 256维 + subject_id + 指纹(fingerprint)"]
    C --> D{"指纹候选前置: GET semfp:v1:<指纹> 直读候选?"}
    D -- 命中候选 --> E["/rerank 复核(reason=ok?)/取答"]
    E -- 通过 --> Z["返回缓存答案 source=cache"]
    D -- 无同指纹 ---> F["VSEARCH 256 <向量> TopK(默认30) 暴力余弦候选"]
    F --> G["对每个候选 GET<问题> + /rerank <decision>(reason) 复核"]
    G -- reason 为拒(conflict/unknown) --> H["试下一个候选"]
    H --> G
    G -- reason=ok & score 达标 --> Z
    G -- 全不过 --> X
```

**引擎路由（关键点）**：
- kvstore 命令按首字母路由：`SET`/`GET` → 数组引擎 `global_array`；`HSET`/`HGET` → **hash 引擎 `global_hash`**，二者互不可见。
- 明文 Q-A 走 `SET`（数组）；向量索引用 `HSET semcache:<问题原文>`（hash）——因为 `VSEARCH` 只扫 `global_hash` 的 `semcache:` 前缀（`kvs_vector.c:47-97`）；VSEARCH 命中返回的 `semcache:<缓存问题原文>` → 剥前缀得问题 → `GET <问题>` 取明文回答。
- **语义指纹（安全落地）**：`semfp:v1:<fingerprint>` 前缀写数组引擎，值 = 候选问题文本指针（**不把答案写入指纹键**）。同措辞同指纹（subject/intent/language/operation/output_type/residual 全同才同指纹）→ 精确重复/近重复问题经指纹候选前置直读，**省去一次 VSEARCH 余弦**；语义相近但槽位不同的改写才落到 TopK 回退路径逐步 /rerank 复核。指纹带 schema/parser_version/ontology_version 版本化（旧语义直接 miss 不回退答案）。
...
**kvstore VSEARCH**：`VSEARCH <dim> <query_bytes> <topk>`（`kvstore.c`）→ `kvs_vector_search`（`kvs_vector.c`：parse_vec 校验 `[dim][vec]` + cosine + 暴力 top-k 冒泡）。record 已从 `[qlen][q][alen][a][dim][vec]` 简化为 `[dim][vec]`。service 侧 `top_k` 配置默认 **30**（`semcache.defaultTopK` / `dev.config.yaml` `semantic_cache.top_k`），<1 回退 30。

**decision（semantic `/rerank`，Phase1 保守对称硬拒，非交叉编码器）**：`decide(parse(query), parse(cached_query))` 返回 `{score, shared, reason}`；`shared=false` 即保守拒绝并给**可达拒因 reason**（service 透传日志）。依次做
1. **主题**：双路 subject（中文句式 + 中英句式，compact 未命中本体时保空格重跑命中 latin 别名）；`subject_id` 双方都有且不同才拒（`subject_conflict`）；任一缺 subject_id/subject_text 才按 **unknown_subject** 保守拒（不依赖向量弱匹配）；
...
**为什么拆成两个独立 Python 服务**：token 计数依赖 OpenAI 官方的 `tiktoken`（BPE 表），语义检索依赖中文分词引擎 `jieba` 产出词面嵌入 / 规则 rerank。二者职责不同、变更节奏不同，拆开便于单独按模型扩展、单独水平扩展，与 Go 微服务解耦。**语义职责自 tokenizer 迁入 semantic（2026-09-03 Phase 0）**。
...
> 提供 `/embed`、`/rerank`、`/healthz`；256 维 FNV 哈希词面嵌入，依赖 nuxt+jieba，**无 tiktoken**（语义职责 2026-09-03 Phase 0 自 tokenizer 迁入）。

- **/embed 嵌入（256 维加权词面，FNV 哈希）**：`embed_text`（`embedding.py`）——主题桶用 **canonical**：parse 给 `subject_id`（别名共享同一 id → "红黑树"/"rbtree" 都落 `SUBJECT:red_black_tree`），`subject_id` 缺失才回退 `subject_text`；①`SUBJECT:<canonical>`×3.0 + `INTENT:<意图>`×2.0（标记桶，拉开不同主题/意图）；②jieba 词 ×1.0（模板词×0）；③相邻字符 bigram ×0.4；L2 归一化。稀疏、无监督、可重复。**非神经网络**（§11.7）。
- **/embed 返回附加字段**：`subject` / `subject_id`（双路主题文本 + 本体 id）、`intent`、`language`、`operation`、`output_type`、`bypass_cache`/`context_dependent`（准入）、`fingerprint`（版本化语义指纹 sha256 v1）、`fingerprint_eligible`（保守准入标志）、`parser_version`/`ontology_version`。
- **/rerank = decision 规则校验**：`semantic.py` 薄路由调 `decision.decide(parse(q),parse(c))`→ 返回 `{score, shared, reason}`。**双路 subject + 中英句式解析、对称语言敏感、保守 operation、残差(residual)复核**，按 subject→language→operation→intent→residual 顺序保守对称硬拒；`shared=false` 时 `reason` 为可达拒因（subject/language/operation/intent/constraint_conflict / unknown_subject / unknown_intent），service 透传日志（`semcache` 命中必经 decision 复核）。得分/阈值（cos 0.35 / TopK 30）与缓存编排见 [4.2](#42-语义缓存cachequery--cachewrite替换腾讯-vectordb)。
- **/healthz**：健康探针，供 Swarm healthcheck 用（`http://127.0.0.1:3003/healthz`，200 即存活）。
- 框架：`nuxt`（Python Web 微框架），`--workers 2`；Swarm 配置见 [8.1](#81-生产docker-swarmai-chat-stack)。
...
**结论**：词面嵌入有天花板。分两类处理：**软件/代码/定义题**（subject 可靠）缓存命中可靠；**通用自由 how-to**（怎么减肥/推荐电影）无法正则切主题 → **方案 A 安全收窄不缓存**（永远走 LLM，杜绝误命中）。彻底覆盖通用问答需换真实语义模型（bge-small-zh），规则保留作安全边界。
...
- `semantic/semantic.py` /embed /rerank /healthz（语义检索，256 维 FNV 哈希，依赖 nuxt+jieba，无 tiktoken）
